...
func startTestRound(cs *consensus.State, height int64, round int32) {
...
func addVotes(to *consensus.State, votes ...*types.Vote) {
	for _, vote := range votes {
		to.PeerMsgQueue <- consensus.MsgInfo{Msg: &consensus.VoteMessage{vote}}
	}
}
...
func validatePrevote(t *testing.T, cs *consensus.State, round int32, privVal *ValidatorStub, blockHash []byte) {
...
func validateLastPrecommit(t *testing.T, cs *consensus.State, privVal *ValidatorStub, blockHash []byte) {
	votes := cs.LastCommit
...
func subscribeToVoter(cs *consensus.State, addr []byte) <-chan tmpubsub.Message {
...
func newState(state sm.State, pv types.PrivValidator, app abci.Application) *consensus.State {
...
func RandState(config *cfg.Config, nValidators int) (*consensus.State, []*ValidatorStub) {
	// Get State
...
func randConsensusState(
	...
	nValidators int,
	testName string,
	tickerFunc func() consensus.TimeoutTicker,
	appFunc func() abci.Application,
	configOpts ...func(*cfg.Config),
) ([]*consensus.State, CleanupFunc) {
...
func NewMockTickerFunc(onlyOnce bool) func() consensus.TimeoutTicker {
	return func() consensus.TimeoutTicker {
		return &mockTicker{
			c:        make(chan consensus.TimeoutInfo, 10),
			onlyOnce: onlyOnce,
		}
	}
}
...
func (m *mockTicker) ScheduleTimeout(ti consensus.TimeoutInfo) {
	m.mtx.Lock()
	defer m.mtx.Unlock()
...
func (m *mockTicker) Chan() <-chan consensus.TimeoutInfo {
	return m.c
}
